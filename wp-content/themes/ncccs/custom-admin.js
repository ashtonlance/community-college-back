document.addEventListener("mouseover", function (e) {
  ...
  if (e.target.closest('.block-editor-block-types-list__item')) {
    ...
    const blockClass = blockClasses.find(cls => cls.startsWith("editor-block-list-item-"));

    // If such a class is found, extract the name from it
    if (blockClass) {
      const blockName = blockClass.replace("editor-block-list-item-", "");
      // Get the image URL for this block
      const imageUrl = wp.data.select('core/blocks').getBlockType(blockName.replace("-","/"))?.attributes?.previewImage?.default;
